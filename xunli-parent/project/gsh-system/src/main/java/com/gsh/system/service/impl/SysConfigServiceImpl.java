...
public class SysConfigServiceImpl implements ISysConfigService
{
    ...
    public void init()
    {
        ...
        {
            CacheUtils.put(getCacheName(), getCacheKey(config), config.getConfigValue());
        }
    }
    ...
    public int insertConfig(SysConfig config)
    {
		...
		if (FactoryUtils.isPlatformFactoryCode() && "N".equals(config.getConfigType())) {
			List<SysConfig> configList = configMapper.selectDistinctFactoryCode();
			for (SysConfig sysConfig : configList) {
				config.setFactoryCode(sysConfig.getFactoryCode());
    ...
    {
        return Constants.SYS_CONFIG_KEY + CacheUtils.getFactoryCode() + configKey;
    }
     ...
     * @return 缓存键key
     */
    private String getCacheKey(SysConfig config)
    {
        return Constants.SYS_CONFIG_KEY + config.getFactoryCode() + config.getConfigKey();
    }
}
