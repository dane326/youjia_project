/**
 * 【文本比较插件】
 * 传递两个参数domId1、domId2，以dom1为基准进行比较。
 * 0）dom1和dom2不能都为空；
...
(function(window,document){
    function DomTextCompare(domId1,domId2){
        ...
        else if(!dom1 || dom1.innerHTML == ''){
            //dom1为空：新增
            dom2.style.color = '#2fd052';
        }else if(!domId2 || !dom2.innerHTML){
            //dom2为空：删除
            dom1.style.color = '#FF6347';//AEE194
    ...
    function _eq(op) {
        ...
        if(!op.value1_style){
            ...
            //op.value1_style="background-color:#FEC8C8;text-decoration:line-through;"; //显示删除线
        }
        if(!op.value2_style){
            op.value2_style="background-color:#FEC8C8;";//C5FF7E//FFE835
        }
        if(!op.eq_min){
            op.eq_min=3;
        }
    ...
    window.CompareDomText = DomTextCompare;
})(window,document);