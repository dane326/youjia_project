...
public class SysPasswordService
{
    ...
    public void validate(SysUser user, String password)
    {
        ...
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(user.getFactoryCode(), loginName, Constants.LOGIN_FAIL, MessageUtils.message("user.password.retry.limit.exceed", maxRetryCount)));
            throw new UserPasswordRetryLimitExceedException(Integer.valueOf(maxRetryCount).intValue());
        }

        if (!matches(user, password))
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(user.getFactoryCode(), loginName, Constants.LOGIN_FAIL, MessageUtils.message("user.password.retry.limit.count", retryCount)));
            loginRecordCache.put(loginName, retryCount);
