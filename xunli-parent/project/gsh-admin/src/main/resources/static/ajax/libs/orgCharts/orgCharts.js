...
function OrgCharts() {
	...
	this.init = function(data) {
		try {   //尝试初始化
			...
			try {
				isFunction = typeof(eval(data.onDelete)) == "function";
			} catch(e) {}
			if(isFunction) {
				orgCharts.onDelete = data.onDelete;
			}
			...
			for(var x in nodes) {
				...
				contentSpan.onmouseup = function(oEvent) {
					...
					if(oEvent.button == 2) {
						...
						if(document.getElementById("org_menu_add")) {
		...
		if(orgCharts.menu.length > 0) {
			...
			for(var x in orgCharts.menu) {
				...
				switch(orgCharts.menu[x]) {
					case 'add':
						img.src = ctx+"ajax/libs/orgCharts/svg/plus-circle.svg";
					...
					case 'delete':
						...
						img.src = ctx+"ajax/libs/orgCharts/svg/minus-circle.svg";
						items.appendChild(img);
						img.title = '删除节点';
						img.onclick = function() {
							//获取标记
							...
							orgCharts.onDelete(dataTemp,data,tabs);
						}
						break;
					case 'cut':
						img.id = 'org_menu_cut';
						img.src = ctx+"ajax/libs/orgCharts/svg/cut.svg";
						items.appendChild(img);
						img.title = '剪切';
						paste = true;
						img.onclick = function() {
					...
					case 'copy':
						...
						img.src = ctx+"ajax/libs/orgCharts/svg/copy.svg";
						items.appendChild(img);
						img.title = '拷贝';
						paste = true;
						img.onclick = function() {
					...
					case 'absorbed':
						...
						img.src = ctx+"ajax/libs/orgCharts/svg/absorbed.svg";
						items.appendChild(img);
						img.title = '专注模式';
						img.onclick = function() {
							closeOrgMenue();
						...
						if(orgCharts.resetD.length > 0) {
							...
							img2.src = ctx+"ajax/libs/orgCharts/svg/back.svg";
							...
							img3.src = ctx+"ajax/libs/orgCharts/svg/backAll.svg";
					...
					case 'edit':
						...
						img.src = ctx+"ajax/libs/orgCharts/svg/edit.svg";
						items.appendChild(img);
						img.title = '编辑';
						paste = true;
						img.onclick = function() {
			...
			if(paste) {
				...
				img.style.cursor = 'pointer';
				img.style.margin = '2px';
				img.src = ctx+"ajax/libs/orgCharts/svg/paste.svg";
				items.appendChild(img);
				img.title = '粘贴';
				img.onclick = function() {
					orgCharts.targetF.parentNode.parentNode.style.border = 'none';
			...
			img.style.cursor = 'pointer';
			img.style.margin = '2px';
			img.src = ctx+"ajax/libs/orgCharts/svg/close-circle.svg";
			items.appendChild(img);
			img.title = '关闭';
			img.onclick = function() {
				closeOrgMenue();
